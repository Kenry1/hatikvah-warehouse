...
const USE_SHARED_DRIVE = true; // Enable shared drive support
...
class GoogleDriveService {
  ...
  async setupSharedDrive() {
    try {
      ...
      console.log('   drive-uploader@sunlit-cab-471213-k9.iam.gserviceaccount.com');
  ...
  async createSharedDrive(driveName) {
    try {
      if (!this.accessToken) {
  ...
  createMultipartBody(metadata, base64Data, mimeType) {
  ...
  async uploadFileResumable(file, metadata) {
    try {
      if (!this.accessToken) {
        await this.initializeDrive();
      }
