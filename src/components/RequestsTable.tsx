...
import { getUserList } from "@/lib/firestoreHelpers";
...
export function RequestsTable({ requests, onViewDetails, companyId, variant = 'desktop' }: RequestsTableProps) {
  const [userList, setUserList] = useState<User[]>([]);
  useEffect(() => {
    if (companyId) {
      getUserList(companyId).then(setUserList).catch(() => setUserList([]));
    }
  }, [companyId]);
